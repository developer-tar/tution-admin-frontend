...
const StudentAnnouncements = () => {
  ...
  const fetchAnnouncements = async () => {
    try {
      ...
      console.log("[Student Announcements] API Response:", {
      ...
      if (response.data && response.data.success !== false) {
        ...
        if (Array.isArray(data)) {
          ...
          console.log("[Student Announcements] Direct array response, count:", announcementsList.length);
        } else if (data && typeof data === 'object') {
          // Paginated response from Laravel
          if (Array.isArray(data.data)) {
            announcementsList = data.data;
            console.log("[Student Announcements] Paginated response (data.data), count:", announcementsList.length);
          } else if (Array.isArray(data.announcements)) {
            announcementsList = data.announcements;
            console.log("[Student Announcements] Paginated response (data.announcements), count:", announcementsList.length);
          } else {
            // Try to find any array property
            for (const key in data) {
              if (Array.isArray(data[key])) {
                announcementsList = data[key];
                console.log(`[Student Announcements] Found array in property '${key}', count:`, announcementsList.length);
                break;
              }
            }
            if (announcementsList.length === 0) {
              console.warn("[Student Announcements] No array found in response data:", data);
          ...
          console.log("[Student Announcements] No data in response (null/undefined)");
          announcementsList = [];
        } else {
          console.warn("[Student Announcements] Unexpected data type:", typeof data, data);
          announcementsList = [];
        }
        ...
        const filteredAnnouncements = announcementsList.filter((announcement) => {
          ...
          const matchesAudience =
            targetAudience.includes("student") ||
        ...
        console.log("[Student Announcements] Final counts - Today:", sortedToday.length, "Other:", sortedOther.length, "Total:", sortedToday.length + sortedOther.length);
      } else {
        console.warn("[Student Announcements] API returned success=false or no data:", response.data);
        setAnnouncements([]);
        setTodayAnnouncements([]);
        setOtherAnnouncements([]);
      }
    } catch (err) {
      console.error("[Student Announcements] Error fetching announcements:", err);
      console.error("[Student Announcements] Error details:", {
        message: err.message,
