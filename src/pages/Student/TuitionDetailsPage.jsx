...
const dropdownParams = ["Subjects"];

const TuitionDetailsPage = () => {
  const { control, setValue } = useForm();

  const { dropdowns, loading: dropdownLoading } = useCommonDropdowns(dropdownParams);
  ...
  // Set default filter values when data is available
  useEffect(() => {
    if (!filters.subject_id && dropdowns.Subjects?.length > 0) {
      const defaultSubject = dropdowns.Subjects[0]?.id;
      handleFilterChange('subject_id', String(defaultSubject));
      setValue('subject_id', String(defaultSubject));
    }
