...
const TopicTest = () => {
  ...
  useEffect(() => {
    ...
    if (!token || role?.toLowerCase() !== 'student') {
      toast.error('Unauthorized access. Please login as a student.');
      navigate('/login');
      return;
    }
  ...
  // Fetch test data with security validation
  useEffect(() => {
    if (!isAuthorized || !test_id) return;

    const fetchTestData = async () => {
      try {
        const response = await api.get(`/student/topic/test/${test_id}`);
    ...
    fetchTestData();
  }, [test_id, navigate, isAuthorized]);

  // Timer effect with security checks
  useEffect(() => {
    let timer;
    if (testStarted && timeRemaining > 0 && isAuthorized) {
      timer = setInterval(() => {
  ...
  }, [testStarted, timeRemaining, isAuthorized]);

  // Prevent unauthorized access
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden && testStarted) {
        toast.warning('Test paused due to tab switch. Please stay focused on the test.');
      }
    };
  ...
  const handleAnswerChange = (questionId, optionId) => {
    if (!isAuthorized || !testStarted) return;
  ...
  const handleSubmitTest = async () => {
    ...
    try {
      ...
      const response = await api.post(`/student/topic/test/${test_id}/submit`, submitData);
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh' }}>
          <Card sx={{ p: 4, textAlign: 'center', maxWidth: 400 }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              You need to be logged in as a student to access this test.
            </Typography>
        ...
        <Fade in timeout={400}>
          ...
          >
            <Typography variant="body2">
              <strong>Secure Test Environment:</strong> This test is monitored for security. 
              Please do not switch tabs or leave this page during the test.
            </Typography>
          </Alert>
        </Fade>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, mb: 3 }}>
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  <Typography variant="body1" sx={{ opacity: 0.9 }}>
                    Topic Test Instructions & Overview
                  </Typography>
                </Box>
              </Box>
            </CardContent>
              ...
              <Card sx={{ height: 'fit-content' }}>
                <CardContent sx={{ p: 4 }}>
                  ...
                  <Box sx={{ mb: 4 }}>
                    ...
                    <Paper sx={{ p: 3, bgcolor: 'error.50', border: '1px solid', borderColor: 'error.200', mb: 3 }}>
                      ...
                      <Typography variant="body2" color="error.dark">
                        • Do not switch tabs or minimize the browser during the test<br/>
                        • Do not use external resources or assistance<br/>
                        • Ensure you have a stable internet connection<br/>
                        • Complete the test in one sitting without interruption<br/>
                        • Any suspicious activity may result in test termination
                      </Typography>
                    </Paper>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              {currentQuestionIndex === testData.test.questions.length - 1 ? (
                <Button
                  variant="contained"
                  color={testCompleted ? "default" : "success"}
                    ...
                    <Grid item xs={6} sm={3}>
                      <Box sx={{ textAlign: 'center' }}>
                        <Chip
                          ...
                          color={testResults.is_completed ? 'success' : 'warning'}
                          variant="filled"
                        />
